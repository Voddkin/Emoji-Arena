...
# Locate CardDatabase and isolate it
start_idx = js.find('const CardDatabase = [')
end_idx = js.find('];', start_idx) + 2

cards_text = js[start_idx:end_idx]

# Perform translations on name and desc strings only
# We will do regex replaces specifically looking for "name: '...'" and "desc: '...'"

translations = {
    # BASIC / TROOPS
    "name: 'Goblin Grunt', desc: 'A basic melee fighter.'": "name: 'Brutamontes Goblin', desc: 'Um lutador corpo-a-corpo básico.'",
    ...
    # Check for keywords inside cards array
    "keywords: ['taunt']": "keywords: ['provocar']",
    "keywords: ['rush']": "keywords: ['investida']",
    "keywords: ['lifesteal']": "keywords: ['vampirismo']",
    "keywords: ['poison']": "keywords: ['veneno']",
    "keywords: ['shield']": "keywords: ['escudo']",
}

for eng, pt in translations.items():
    cards_text = cards_text.replace(eng, pt)

# We also need to map the tribes constants globally
# Let's replace the whole block back
new_js = js[:start_idx] + cards_text + js[end_idx:]
