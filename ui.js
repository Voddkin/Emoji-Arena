...
function updateBattleUI() {
    ...
    ['player', 'opponent'].forEach(side => {
        ...
        if (!handEl) return;
        handEl.innerHTML = '';
        gameState[side].hand.forEach((card, i) => {
            const wrap = document.createElement('div');
            wrap.innerHTML = createCardHTML(card, side === 'opponent' && !card.revealed);
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (!slotEl) continue;
            slotEl.innerHTML = '';
            let boardCard = gameState[side].board[i];
            if (boardCard) {
                const wrap = document.createElement('div');
                wrap.innerHTML = createCardHTML(boardCard);
                let cEl = wrap.firstElementChild;
                cEl.classList.add('on-board');
                if (side === 'player' && !boardCard.exhausted && gameState.isPlayerTurn) {
                    cEl.classList.add('can-attack');
                    cEl.onclick = () => {
                        if (typeof executeAttack === 'function') executeAttack(i, null, true);
                    };
                }
                slotEl.appendChild(cEl);
                slotEl.className = 'board-slot filled';
            } else {
                slotEl.className = 'board-slot empty';
            }
        }
        ...
        const credEl = document.getElementById(side + '-credits');
        if (credEl) credEl.innerText = gameState[side].credits + '/' + gameState[side].maxCredits;
    });
}
